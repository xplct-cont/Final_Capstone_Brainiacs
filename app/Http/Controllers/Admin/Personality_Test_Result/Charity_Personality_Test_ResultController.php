...
class Charity_Personality_Test_ResultController extends Controller
{
    ...
    public function index($id){
        ...
        $personality_test_result_charity = Personality_Test_Result::with(['student'])->where('student_id', '=', $id)->get();
        
  
        $student_cha = Student::find($id);
        return view('admin.student.Charity.Personality_Test_Result.index', compact('personality_test_result_charity', 'student_cha'));
      }
      ...
      {
          $charityStudents_personality_test_result = Personality_Test_Result::findOrFail($id);
          $pdf = PDF::loadVIew('pdf.charity-personality_test_result', [
              'personality_test_results' => $charityStudents_personality_test_result
          ]);
  
          return $pdf->download('Charity-Personality Test Result.pdf');
      }
}