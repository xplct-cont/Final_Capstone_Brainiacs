...
<div class="p-1">
    ...
    <h1 class="text-dark p-3"
        ...
        Counseling Anecdotal Records of {{ $student_cha->lastname }}, {{ $student_cha->firstname }} from
        ...
        <div class="card">
            <div class="card-header text-center">
                ...
                <div class="d-flex justify-content-end">
                    <a href="{{ url('show-student-charity/' . $student_cha->id . '/counseling_anecdotal_record_charity/create/') }}"
                ...
                <table class="table table-striped table-sm text-dark text-center bg-light">
                    ...
                    <tbody>
                        @forelse ($counseling_anecdotal_charity as $coun_anec_cha)
                            <tr>
                                <td class="text-dark">{{ $coun_anec_cha->date_time_called->format('F d,  Y - g:i A') }}</td>
                                <td class="text-dark">{{ $coun_anec_cha->reasons_for_contact }}</td>
                                <td><a href="{{ url('/show-student-charity/'.$coun_anec_cha->student->id.'/counseling_anecdotal_record_charity/' . $coun_anec_cha->id) }}"
                                        class="btn btn-xs "><i class="fas fa-search text-info"></i></a></td>
                                <td><a href="{{ url('delete_counseling_anecdotal_record_charity/' . $coun_anec_cha->id) }}"
